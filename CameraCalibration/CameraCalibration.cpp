...
int readFilenames(vector<string>& filenames, const string& directory) {
    // Open txt file
    std::string txtfile = directory + "images_list.txt";
    std::ifstream infile;
    infile.open(txtfile);
...
int main(int argc, const char* argv[]) {
    ...
    int board_w = 0;          // corner number in width
    int board_h = 0;          // corner number in height
    ...
    board_w = atoi(argv[1]);       // input first parameter
    board_h = atoi(argv[2]);       // input second parameter
    int n_boards = atoi(argv[3]);  // how many boards max to read(set value bigger than image number), third parameter
    delay = atof(argv[4]);         // milisecond delay, forth parameter
    image_sf = atof(argv[5]);      // fifth parameter
    int board_n = board_w * board_h;                 // number of corners in chessboard
    cv::Size board_sz = cv::Size(board_w, board_h);  // width and height of the board

    string folder = argv[6];       // image folder(include txt file writed image filenames)
    cout << "Reading in directory " << folder << endl;
    vector<string> filenames;
    int num_files = readFilenames(filenames, folder);// read image filename list from txt
    cout << "... Done. Number of files = " << num_files << "\n" << endl;

    // PROVIDE PPOINT STORAGE
    //
    vector<vector<cv::Point2f> > image_points;      // for saving corner points in 2D images
    vector<vector<cv::Point3f> > object_points;     // for saving corner points in 3D place
    ...
    double err = cv::calibrateCamera(
        ...
        cv::CALIB_ZERO_TANGENT_DIST | cv::CALIB_FIX_PRINCIPAL_POINT);       // in origin image size
